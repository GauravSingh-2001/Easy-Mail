...
const ComposeDialog = ({ isOpen, onClose, draft }) => {
  ...
  const fetchContacts = async () => {
    try {
      const response = await fetch(`https://easymail-0cfn.onrender.com/api/savedEmails`);
  ...
  const sendMail = async (e) => {
    ...
    try {
      const response = await fetch(`https://easymail-0cfn.onrender.com/api/sendmail`, {
        method: 'POST',
        headers: {
  ...
  const saveDraft = async () => {
    ...
    try {
      const response = await fetch(`https://easymail-0cfn.onrender.com/api/emails/draft`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleCancel = async () => {
    ...
    if (confirmSave) {
      ...
      if (draft && draft._id) {
        try {
          const response = await fetch(`https://easymail-0cfn.onrender.com/api/emails/${draft._id}`, {
            method: 'DELETE',
          });
          if (!response.ok) {
